...
```
usage: sbomaudit [-h] [-i INPUT_FILE] [--offline] [--cpecheck] [--purlcheck] [--disable-license-check] [--age AGE] [--maxage MAXAGE] [--allow ALLOW] [--deny DENY] [--verbose] [--direct-only] [--debug] [-o OUTPUT_FILE] [--export-metadata EXPORT_METADATA] [-V]
...
Input:
  ...
  --direct-only         only analyse direct dependencies, ignoring transitive ones
...
Output:
  ...
  --export-metadata EXPORT_METADATA
                        export package metadata to CSV file
```
...
The `--direct-only` option is used to analyse only direct dependencies, ignoring transitive ones. This is useful when you want to focus on the packages that are explicitly declared as dependencies in your project, rather than the full dependency tree. Direct dependencies are identified through the SBOM's relationship data (DESCRIBES and DEPENDS_ON relationships).

The `--output-file` option is used to control the destination of the output generated by the tool. The
default is to report to the console but can be stored in a file (specified using `--output-file` option).

The `--export-metadata` option is used to export collected package metadata to a CSV file. The CSV file contains the following fields:
- `name` - Package name
- `sbom_version` - Version specified in the SBOM
- `latest_version` - Latest available version from the package registry
- `ecosystem` - Package ecosystem (e.g., pypi, composer, npm)
- `license` - License information from the package registry
- `description` - Package description

This option requires network access to fetch metadata from package registries. 
